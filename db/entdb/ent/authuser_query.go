...
func (auq *AuthUserQuery) QueryUser() *UserQuery {
	query := (&UserClient{config: auq.config}).Query()
...
func (auq *AuthUserQuery) FirstIDX(ctx context.Context) int {
...
func (auq *AuthUserQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (auq *AuthUserQuery) WithUser(opts ...func(*UserQuery)) *AuthUserQuery {
	query := (&UserClient{config: auq.config}).Query()
...
func (auq *AuthUserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*AuthUser, error) {
	...
	if query := auq.withUser; query != nil {
		if err := auq.loadUser(ctx, query, nodes, nil,
			func(n *AuthUser, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (auq *AuthUserQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*AuthUser, init func(*AuthUser), assign func(*AuthUser, *User)) error {
...
func (auq *AuthUserQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(authuser.Table, authuser.Columns, sqlgraph.NewFieldSpec(authuser.FieldID, field.TypeInt))
	_spec.From = auq.sql
	...
	if fields := auq.ctx.Fields; len(fields) > 0 {
		...
		if auq.withUser != nil {
			_spec.Node.AddColumnOnce(authuser.FieldUserID)
		}
	}
...
func (auq *AuthUserQuery) ForUpdate(opts ...sql.LockOption) *AuthUserQuery {
...
func (auq *AuthUserQuery) ForShare(opts ...sql.LockOption) *AuthUserQuery {
	if auq.driver.Dialect() == dialect.Postgres {
		auq.Unique(false)
	}
	auq.modifiers = append(auq.modifiers, func(s *sql.Selector) {
