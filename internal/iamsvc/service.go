...
type (
	...
	AdminCredentials = iam.AdminCredentials
	Role             = iam.Role
...
// Returns iam.ErrUserDoesNotExist if user does not exist, iam.ErrUserAlreadyExists if username exists,
// or iam.ErrInvalidCredentials if creds invalid.
...
func (i *IAM) checkUsernameFree(
	...
	if exists {
		...
		return iam.ErrCredentialsAlreadyExist
	}
...
// Returns iam.ErrInvalidCredentials if the token is not valid.
...
// ImWatermelon parses tokenString, returns Identity or iam.ErrInvalidToken.
...
func (i *IAM) retrieveUserIdentity(
	...
	if err != nil {
		...
		return Identity{}, iam.ErrInvalidToken
	}
	...
	case ent.IsNotFound(err):
		rec.Set("found", false)
		return Identity{}, iam.ErrUserNotFound
	case err != nil:
...
// DropCredentials deletes credentials by userID; returns iam.ErrUserNotFound if credentials missing,
// or iam.ErrUserDoesNotExist if the user doesn't exist.
